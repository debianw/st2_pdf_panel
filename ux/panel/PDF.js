...
Ext.define('Ext.ux.panel.PDF', {
  ...
  initialize: function() {
    ...
    // Canvas
    items.push({
      html: '<figure><canvas class="pdf-page-container"></canvas></figure>'
    });
    ...
    if (!config.hidePagingtoolbar) {
      items.push({
        ...
        docked: 'bottom',
        title: Ext.String.format(this.getPageText(), 1, 1),
        items: this.getPagingItems()
      });
    }
    ...
    // Set title
    this.down('toolbar').setTitle(this.getTitle());
  ...
  setBlobdata: function(blob) {
    var me = this;
    if (blob !== null) {
      me.config.src = null;
    }
    me.config.blobdata = blob;
    me.loadPdf();
    return me;
  },
  ...
  loadPdf: function(src, blob, password) {
    var me = this;

    src = src || me.getSrc();
    blob = blob || me.getBlobdata();
    password = password || me.getPassword();

    if ((!src && !blob) || me.isLoading) {
      return me;
    }
    ...
    if (me.canvasEl) {
      ...
      if (typeof src === 'string') { // URL
        params.url = src;
      } else if (blob && 'byteLength' in blob) { // ArrayBuffer
        params.data = blob;
      }
  ...
  renderPage: function(num, callback) {
    ...
    if (!me.getHidePagingtoolbar()) {
      ...
      toolbar.down('#prev').setDisabled(currPage === 1 || isEmpty);
      toolbar.down('#next').setDisabled(currPage === pageCount || isEmpty);
    }
